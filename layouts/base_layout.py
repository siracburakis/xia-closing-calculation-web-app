...
def base_layout(content_function, current_url):
    ...
    # Sidebar (left drawer)
    with ui.left_drawer(fixed=False).classes('sidebar sidebar-drawer').style('width: 100px; padding: 10px;').props('bordered') as left_drawer:
    ...
    # Main content alanı
    with ui.element("main").classes("main-content flex-1 p-4 w-full"):
        content_function()
